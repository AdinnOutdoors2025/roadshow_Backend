...
/*  marks the entry it replaces entryStatus:"removed" and stamps the new entry's */
/*  replacesEntryId with the old entry's _id. A "current slot" is any entry     */
/*  that is NOT entryStatus:"removed" — there is always exactly one per live    */
/*  chain — and its full chain is reconstructed by walking replacesEntryId      */
...
const resolveVehicleSlots = (onRoadExecutionArray, vehicleIndex) => {
  ...
  return entries
    .filter((entry) => entry.entryStatus !== "removed")
    .map((head) => {
